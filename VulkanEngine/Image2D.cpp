...
namespace caaVk {
    ...
    /*
    이미지 생성 방법
	- format: 이미지의 픽셀 형식 (예: RGBA8, RGBA16F, RGBA32F 등)
	- width, height: 이미지의 가로 및 세로 크기
	- sampleCount: 멀티샘플링을 사용할 경우 샘플 수 (예: VK_SAMPLE_COUNT_1_BIT, VK_SAMPLE_COUNT_4_BIT 등)
	- usage: 이미지가 사용될 용도 (예: VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT 등)
	- aspectMask: 이미지의 어떤 부분에 접근할지 (예: VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_ASPECT_DEPTH_BIT 등)
	- mipLevels: 이미지의 MIP 레벨 수 (0이면 자동 계산)
	- arrayLayers: 이미지의 배열 레이어 수 (예: 1이면 단일 이미지, 6이면 큐브맵 등)
	- flags: 이미지 생성 시 추가 옵션 (예: VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT 등)
	- viewType: 이미지 뷰의 유형 (예: VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_VIEW_TYPE_CUBE 등)
    
    1. Image2D 만듬. -> 단순한 설계도
    2. vkGetImageMemoryRequirements를 통해 메모리가 얼마나 필요한지 확인
    3. vkAllocateMemory로 메모리 할당
	4. vkBindImageMemory로 이미지와 메모리 연결
    5. VkImage라는 데이터 덩어리를 셰이더나 파이프라인이 읽을 수 있게 "통로" 혹은 "해석기" ViewImage 생성
	6. ResourceBinding에 이미지와 뷰 정보 업데이트 -> DescriptorSet에서 사용할 수 있도록 준비
    */
        ...
        VkImageCreateFlags flags, VkImageViewType viewType)
    {
		...
		// VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT : GPU 전용 고속도로에 위치한 메모리 (CPU 접근 불가)
